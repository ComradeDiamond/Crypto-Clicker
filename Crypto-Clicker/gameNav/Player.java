...
public class Player 
{
    ...
    /**
     * Forcibly swaps coins.
     * @param coinName The name of the coin to swap
     * @return Whether the coin exists
     */
    public static boolean swapCoin(String coinName)
    {
        for (Coin coin : Player.coinArr)
        {
            if (coin.getName().equalsIgnoreCase(coinName))
            {
                Player.currCoin = coin;
                return true;
            }
        }

        System.out.println("Sorry this coin does not exist.");
        return false;
    }
}
